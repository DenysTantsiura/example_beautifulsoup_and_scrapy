...
def get_soup_from_url(url: str, parser: str = 'lxml') -> BeautifulSoup:  # перетворимо відповідь від сервера у формат lxml
...
def gather_information(url: str) -> tuple[list]:
    ...
    for about in abouts:
        soup_about = get_soup_from_url(about)
...
def main() -> None:
    ...
    save_to_json('jsons_files/authors.json', authors_in_json)
    save_to_json('jsons_files/quotes.json', quotes_in_json)
